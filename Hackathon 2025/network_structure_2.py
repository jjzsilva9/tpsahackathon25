...
class Network:
        ...
        def update_frame(frame_num):
            ...
            if frame_num > 0:
                ...
                for road_id_sink in road_ids_sink:
                    road = self.roads_dict.get(road_id_sink)
                    if road:
                        road.density -= 10  # Remove cars, avoid negative density

                for road_id_source in road_ids_source:
                    road = self.roads_dict.get(road_id_source)
                    if road:
    ...
    def save_traffic_animation(self, filename="traffic_animation.gif", num_steps=20, interval=800, grid_layout=True, rows=10, cols=10):
        """
        Create and save traffic flow animation as a GIF file with proper sink/source monitoring.
        ...
        # Setup plot with three subplots for monitoring
        ...
        total_densities = []
        sink_densities_over_time = {road_id: [] for road_id in road_ids_sink}
        source_densities_over_time = {road_id: [] for road_id in road_ids_source}
        cars_added_per_step = []
        cars_removed_per_step = []
        ...
        def update_frame(frame_num):
            ...
            if frame_num > 0:
                ...
                # Track cars added and removed for mass conservation
                cars_added = 0
                cars_removed = 0
                
                # Apply sources and sinks every time step
                # Sinks: Remove cars (destroy density) - but ensure they don't stay at 0
                for road_id_sink in road_ids_sink:
                    road = self.roads_dict.get(road_id_sink)
                    if road:
                        cars_to_remove = min(10, road.density)  # Don't remove more than available
                        road.density = max(0, road.density - cars_to_remove)
                        cars_removed += cars_to_remove
                        
                        # If sink is empty, add some base traffic to keep it functional
                        if road.density < 5:
                            road.density += 5
                            cars_added += 5

                # Sources: Add cars (create density)
                for road_id_source in road_ids_source:
                    road = self.roads_dict.get(road_id_source)
                    if road:
                        road.density += 10  # Add cars
                ...
                cars_added_per_step.append(cars_added)
                cars_removed_per_step.append(cars_removed)
                
                print(f"Step {frame_num}: Added {cars_added} cars, Removed {cars_removed} cars, Net: {cars_added - cars_removed}")
            
            # Record densities for monitoring
            for road_id in road_ids_sink:
                road = self.roads_dict.get(road_id)
                if road:
                    sink_densities_over_time[road_id].append(road.density)
                else:
                    sink_densities_over_time[road_id].append(0)
            
            for road_id in road_ids_source:
                road = self.roads_dict.get(road_id)
                if road:
                    source_densities_over_time[road_id].append(road.density)
                else:
                    source_densities_over_time[road_id].append(0)
            ...
            # Total density plot (middle plot)
            if total_densities:
            ...
            # Sink densities plot (right plot)
            if frame_num > 0:
                steps = range(len(list(sink_densities_over_time.values())[0]))
                for road_id, densities in sink_densities_over_time.items():
                    if densities:  # Only plot if we have data
                        ax3.plot(steps, densities, label=road_id, linewidth=1.5)
                
                ax3.set_xlabel('Time Step')
                ax3.set_ylabel('Sink Road Density')
                ax3.set_title('Sink Road Densities Over Time')
                ax3.grid(True, alpha=0.3)
                ax3.legend(fontsize=8, loc='upper right')
                
                # Add warning if sinks are too low
                min_sink_density = min([d[-1] for d in sink_densities_over_time.values() if d])
                if min_sink_density < 5:
                    ax3.text(0.02, 0.02, f"⚠️ Min sink density: {min_sink_density:.1f}", 
        ...
        total_cars_added = sum(cars_added_per_step)
        total_cars_removed = sum(cars_removed_per_step)
        print(f"Total cars added: {total_cars_added}")
        print(f"Total cars removed: {total_cars_removed}")
        print(f"Net change: {total_cars_added - total_cars_removed}")
        print(f"Initial total density: {sum(initial_densities):.1f}")
        print(f"Final total density: {total_densities[-1]:.1f}")
        print(f"Actual change: {total_densities[-1] - sum(initial_densities):.1f}")
